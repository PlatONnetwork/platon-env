...
class Node:
    def __init__(self, config: Config, node_info: NodeInfo):
        ...
        # remote deploy info
        if os.path.isabs(config.DEPLOY_DIR):
            self.node_dir = os.path.join(config.DEPLOY_DIR, self.node_name)
        else:
            self.node_dir = os.path.join(os.path.join(self.pwd, config.DEPLOY_DIR), self.node_name)
        ...
        # node local tmp
        self.local_tmp_dir = os.path.join(config.TMP_NODE_DIR, self.host + '_' + self.p2p_port)
        # make dirs
        # self.make_local_dir()
